...
try {
  // Read questions
  const questionData: QuestionData = JSON.parse(readFileSync(QUESTIONS_PATH, 'utf8'))
  ...
  for (const [index, q] of questions.entries()) {
    ...
    if (!source) {
      continue // Skip questions without a source
    }

    if (source.includes('UoBiA') || source.includes('uobia')) {
      const articleMatch = source.match(/art\.\s(\d+)/)
      if (articleMatch) {
        const article = Number.parseInt(articleMatch[1], 10)
